...
public class Solution_SWEA_1767_프로세서연결하기_이진호 {
    static int N, max, totalCnt, min, map[][];
	static int dr[] = {-1,1,0,0};
	static int dc[] = {0,0,-1,1};
	static ArrayList<int[]> list; // 고려해야하는 코아만 담고 있는 리스트 (가장자리 코어 배제)
	...
	private static boolean isAvailable(int r, int c, int d) {
		...
		while(true) {
			nr += dr[d];
			nc += dc[d];
			if(nr < 0 || nr >= N || nc < 0 || nc >= N) { // 계속해서 해당방향으로 직진을 해서 경계를 벗어남. : 전원 연결 가능
				break;
			}
			if(map[nr][nc] >= 1) { // 코어나 전신이 놓아져 있어서 불가능
				return false;
			}
		}
		return true;
